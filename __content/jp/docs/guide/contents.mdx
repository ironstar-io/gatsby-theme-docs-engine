...
All content are stored under `/content`. To change the root path, modify the following part in `gatsby-config.js`, simply by replace the `path` of it.
...
- `root`: root of the pages should show in the sidebar (based on the path under `/content`)
...
The path of this file is `/content/docs/guide/content.md`. Since the root is `/docs`, all pages under this path will be showing in the sidebar. Some information is not used (i.e. date), so this file don't have it.
